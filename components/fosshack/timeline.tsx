...
export default function Timeline() {
  ...
  return (
    ...
    >
      {/* TITLE */}
      <motion.h2
        className="text-center text-5xl md:text-6xl font-semibold text-foreground mb-20 md:mb-40"
      ...
      >
        TIMELINE
        <span className="block mt-4 text-base font-normal text-foreground/50">
          (1st – 31st March)
        </span>
      </motion.h2>
          ...
          {timeline.map((item, index) => (
            ...
            >
              ...
              <div>
                ...
                {item.date && (
                  /* FIXED: was text-white/50 */
                  <p className="text-sm text-foreground/50 mb-1">{item.date}</p>
                )}
                {item.time && (
                  <p className="text-base text-green-400">{item.time}</p>
                )}
